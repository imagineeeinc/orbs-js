(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.orbsCore = {}));
  ...
  */
  class CaveRenderEngine {
    ...
    draw(scene, fps, antiAliasing, campos, imgStore, shader, renderer, canvasObj) {
  ...
  class newOrbsRenderer {
    ...
    startRenderCycle() {
      ...
      setInterval(() => updateScript(cave,this.canvasObj , this.loop,this.scene, this.updater, this.fps, this.antiAliasing, this.imaStore,this.cameraPos, this.canvasId, this.shader, this), 1000/this.fps)
      function updateScript(cave, canvasObj, loop, scene, update, fps, antiAliasing, imgStore, campos, canvasId, shader, renderer) {
        if (update === true) {
          loop({fps: fps, screen: document.getElementById(canvasId), camera: campos})
          let response = cave.draw(scene, fps, antiAliasing, campos, imgStore, shader, renderer, canvasObj)
          if (response[0] === false) {
            alert(response[1])
