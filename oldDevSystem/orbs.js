(function (global, factory) {
  ...
  exports.keys = {left: 'ArrowLeft', up: 'ArrowUp', right: 'ArrowRight', down: 'ArrowDown'}
  ...
  */
  class CaveRenderEngine {
    ...
    draw(scene, fps, antiAliasing, campos, imgStore, shader, renderer, canvasObj, beforeRender, afterRender, keyboardEvent) {
      ...
      beforeRender({scene, fps, antiAliasing, campos, imgStore, shader, renderer, canvasObj})
      let key = keyboardEvent.event.keyStroke
      keyboardEvent.callback({key, event: keyboardEvent.event, delta: this.deltaTime, scene, onDirection: (arrow)=>{
        let k = keyEvent(key)
      ...
      for (var i=0;i<this.scene.vScene.length;i++) {
        ...
        if (obj.collisionLayer) {
          if (scene.layers.collision[obj.collisionLayer]) {
            ...
            var rect1 = {x: obj.x-((obj.width * obj.scale)/2)+obj.hitbox.center[0], y: obj.y-((obj.height * obj.scale)/2)+obj.hitbox.center[1], width: obj.hitbox.width, height: obj.hitbox.height}
            obj.collision = []
            for (let num=0;num < scene.layers.collision[obj.collisionLayer].length;num++) {
              let curObj = scene.getObj(scene.layers.collision[obj.collisionLayer][num])
      ...
      afterRender({scene, fps, antiAliasing, campos, imgStore, shader, renderer, canvasObj})
      return [true]
    }
    _theDrawers() {
      let drawers = {
  ...
  class newOrbsRenderer {
    ...
    startRenderCycle() {
      ...
      setInterval(() => updateScript(cave,this.canvasObj , this.loop,this.scene, this.updater, this.fps, this.antiAliasing, this.imaStore,this.cameraPos, this.canvasId, this.shader, this, this.onBeforeRender, this.onAfterRender, {callback: this.onKeyboardEvent, event: this.events.keyBoard}), 1000/this.fps)
      function updateScript(cave, canvasObj, loop, scene, update, fps, antiAliasing, imgStore, campos, canvasId, shader, renderer, beforeRender, afterRender, keyboardEvent) {
        if (update === true) {
          loop({fps: fps, screen: document.getElementById(canvasId), camera: campos})
          let response = cave.draw(scene, fps, antiAliasing, campos, imgStore, shader, renderer, canvasObj, beforeRender, afterRender, keyboardEvent)
          if (response[0] === false) {
            alert(response[1])
    ...
    canvasAttactToDom(loc, pre) {
      ...
      document.body.addEventListener('keydown', (e)=>{
        ...
        key += e.key.replaceAll(keys.left, left)
        .replaceAll(keys.right, right)
        .replaceAll(keys.up, up)
        .replaceAll(keys.down, down)
        
        this.events.keyBoard = {key: e.key, ctrlKey: e.ctrlKey, shiftKey: e.shiftKey, altKey: e.altKey}
        this.events.keyBoard.keyStroke = key
      })
      document.body.addEventListener('keyup', (e)=>{
        this.events.keyBoard = {key: '', ctrlKey: false, shiftKey: false, altKey: false}
  ...
  function keyEvent(key) {return key.split('+')}
  exports.ORBS = {
  ...
  exports.CaveRenderEngine = CaveRenderEngine
  exports.still = still
  exports.update = update
  exports.mesh = mesh
  exports.customMesh = customMesh
  ...
  //exports.orbComponents = orbComponents || null
  exports.newOrbsScriptComponent = newOrbsScriptComponent
